1 DEFINT A-Z: OPTION BASE 0: KEY OFF
...
20 'OPL_Touch(c,v): c=channel, v=volume. CHANGES: q,p,o,x,i
...
27 OUT p,&H40+o: x=adl(i,2): GOSUB 29
28 OUT p,&H43+o: x=adl(i,3)
29 OUT p+1,(x OR 63)-v+((x AND 63)*v)\63:RETURN
...
40 'OPL_Reset(). CHANGES: c,q,p,o,x,y
...
50 'OPL_Silence(): Silence all OPL channels
...
102 DIM tkPtr%(100), tkDelay#(100), tkStatus(100)
103 'The same, but for loop-beginning:
104 DIM loPtr%(100), loDelay#(100), loStatus(100)
...
110 DIM ChPatch(15), ChBend#(15), ChVolume(15), ChPanning(15)
...
165 COLOR 8:CLS:txtline=2:? "Press Q to quit; Space to pause"
...
174   hlt=VARPTR(hlt(0)):CALL hlt
175   ' This subroutine^ saves power. It also makes DOSBox faster.
176   ' However, QuickBASIC has a more complicated way for calling
177   ' calling ASM subroutines, so the line 174 will have to be
178   ' disabled when porting to QuickBASIC.
180   y$=INKEY$
181   IF y$=" " THEN paused=1-paused:IF paused THEN ?"Pause":PLAY STOP ELSE ?"Ok":PLAY ON
182 IF y$ <> "q" AND y$ <> CHR$(27) AND y$ <> CHR$(3) THEN 174
190 PLAY OFF: PRINT "End!": GOSUB 50: KEY ON: END

200 'Subroutine: Load MIDI file
210 OPEN filename$ AS #1 LEN=1: FIELD #1, 1 AS b$ 'Open file for random access
...
234 InvDeltaTicks# = playmode% / (240d6 * DeltaTicks)
240 Tempo#         = 1d6 * InvDeltaTicks#
...
253   GOSUB 75: tkDelay#(tk) = x#
254   tkPtr%(tk) = LOC(1)
255   GET #1, y% + TrackLength% 'Skip over the track data
270 NEXT
275 FOR a=0 TO 15: ChVolume(a) = 127: NEXT
281 PLAY ON: ON PLAY(1) GOSUB 300 'Set up periodic event handler
282 began=0:loopstart=1: playwait#=0: PLAY "MLMB"

300 'Subroutine: Timer callback. Called when the PLAY buffer is exhausted.
301 'For each track where delay=0, parse events and read new delay.
302 IF began THEN playwait# = playwait# - 1#
303 WHILE playwait# < .3# : GOSUB 310 : WEND
304 PLAY PLAYmode$: RETURN
...
343 NEXT
344 'Schedule next playevent to be proessed after that delay
...
351 GET #1, tkPtr%(tk)+1: b=ASC(b$)
352 IF b < &HF0 THEN 380
360 'Subroutine for Fx special events
361 '? tk;":";HEX$(b);" at $";hex$(LOC(1))
362 IF b=&HF7 OR b=&HF0 THEN 76 'SysEx. Ignore Varlen.
...
384 ON b\16 - 7 GOTO 400,420,460,470,490,492,495
...
455 GOSUB 520' OPL_Pan with ChPanning
456 GOSUB 530' OPL_Touch with ChVolume
457 chx(c) = 1+(tone+63)MOD 80: chc(c) = 9+(chins(c)MOD 6)
458 LOCATE 20-c,chx(c): COLOR chc(c): ?"#";
459 GOTO 510 ' OPL_NoteOn with ChBend, and return
...
464 ActVol(MidCh,note) = vol
465 LOCATE 20-c,chx(c): COLOR chc(c): ?"&";
466 GOTO 530 'update note volume, and return

470 'Event: Bx Controller change
471 GET #1: ctrlno = ASC(b$): GET #1: value = ASC(b$)
472 IF ctrlno = 6 THEN bendsense# = value/8192#
473 IF ctrlno = 7 THEN ChVolume(MidCh) = value: mop=2:GOTO 500
474 IF ctrlno = 10 THEN 476 'Pan
475 RETURN
476 p=0: IF value<48 THEN p=32 ELSE IF value>79 THEN p=16
477 ChPanning(MidCh)=p
478 mop=3:GOTO 500
...
493 GET #1: PRINT "UNHANDLED Dx":RETURN
...
501 'mop: 1=Update pitches; 2=Update volumes; 3=qUpdate pans
502 x1=ActCount(MidCh)
503 FOR a=1 TO x1
504   note=ActList(MidCh,a)
505   c = ActAdlChn(MidCh,note): ON mop GOSUB 508,509,520
506 NEXT
507 RETURN
508 tone = ActTone(MidCh,note): GOTO 510
509 vol  = ActVol(MidCh,note):  GOTO 530

510 'Subroutine: Update note pitch. Input: c,MidCh,tone. CHANGES q,p,o,x,h#
511 ' 907*2^(n/12) * (8363) / 44100
512 h# = 172.00093*EXP(0.057762265*(tone+ChBend#(MidCh))): GOTO 10 'OPL_NoteOn

520 'Subroutine: Update note pan. Input: c,MidCh
521 chpan(c) = ChPanning(MidCh): GOTO 38 'OPL_Pan

530 'Subroutine: Update note volume. Input: c,MidCh,vol. CHANGES q,p,o,v
531 v = vol * ChVolume(MidCh): GOTO 20 'OPL_Touch
