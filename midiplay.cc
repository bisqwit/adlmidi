...
private:
         ...
         unsigned props_mask)
    {
        ...
        if(props_mask & Upd_Pitch )
        {
            ...
            if(Ch[MidCh].vibrato)
                bend += (Ch[MidCh].vibrato/127.0) * std::sin(VibPos);
...
struct Filter
{
    ...
    void Create(size_t size) { Ptr.resize(size); pos = 0; }
...
static void AddStereoAudio(unsigned long count, int* samples)
{
    ...
    for(unsigned w=0; w<2; ++w)
    {
        ...
        priv.chan[w].reverb.input_fifo.insert(
        priv.chan[w].reverb.input_fifo.end(),
            dry[w].begin(), dry[w].end());
    }
    for(unsigned w=0; w<2; ++w)
    {
        priv.chan[w].reverb.Process(count);
    }
    ...
    for(unsigned long p = 0; p < count; ++p)
        for(unsigned w=0; w<2; ++w)
        {
            float out = ((1 - priv.wetonly) * dry[w][p] +
                .5 * (priv.chan[0].wet[w][p] + priv.chan[1].wet[w][p])) * 32768.0f
                 + average_flt[w];
